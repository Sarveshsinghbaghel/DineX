export function runFrontendAuthTests() {
  ...
  if (!hasPermission('menu.read')) throw new Error('Expected hasPermission("menu.read") to be true');
  if (hasPermission('users.delete')) throw new Error('Expected hasPermission("users.delete") to be false');
